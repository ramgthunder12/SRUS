	...
	<main class="main reservation-page">
        <div class="reservation-page__left">
		    <div class="reservation-page__wrap">
		        <h2 class="title title--h2 text-center">아이디 찾기</h2>
			    <p class="paragraph text-center">아이디 찾기 결과입니다.</p>
			    <div class="row mt-2 mt-sm-4">
			        <div class="col-12">
				        <div class="reservation-card-confirm">
						    <h4 class="title title--h4">아이디</h4>
							<div class="d-flex align-items-end justify-content-center">
							</div>
							<c:if test="${empty rows}">
								<div class="total-reserv">일치하는 아이디 정보가 없습니다</div>
							</c:if>
							<c:if test="${not empty rows}">
								<c:forEach items="${rows}" var="row">
									<div class="total-reserv">${row.id}</div>
								</c:forEach>
							</c:if>
						</div>
					</div>
				    ...
				    <div class="col-12 col-sm-6 mt-2 mt-sm-3 order-2 order-sm-1">
				        <a href="/member/findpassword" class="btn btn__medium btn__second w-100">비밀번호 찾기</a>
				    </div>
			    </div>
			</div>
		</div>
